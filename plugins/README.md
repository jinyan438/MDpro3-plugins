...
5. **开始挑战**使用本体的本地决斗服务器与故事模式自己的 WindBot 通用策略，载入玩家为该角色保存的实际卡组。
   默认故事 AI 使用插件自己的确定性通用策略：常用卡专用规则、发动前收益检查、多步做场与终场比较、按召唤条件保留后续素材、保护已有主力，以及多怪战斗顺序搜索，完全不需要大模型。入口保留为 `Runtime/Features/StoryMode/StoryLuckyExecutor.cs`，实现与测试见 [STORY-LOCAL-AI.md](STORY-LOCAL-AI.md)。也可通过 `ygo-ai` 接入 OpenAI 兼容大模型；故事页面的「模型设置」可编辑 API 地址、密钥并拉取选择模型 ID。模型异常且允许本地回退时由该策略接管；模型配置见 [STORY-AI.md](STORY-AI.md)。
...
**AI 能力边界**：角色共用插件维护的自由构筑通用策略。它会评估启动、检索、干扰、续航、素材成本和战斗斩杀，常用卡另有专用规则；它不是完整卡片脚本模拟器，也不等同于每个主题的专用组合 AI。盖卡按未知信息处理，复杂效果和盖卡会限制战斗预测的准确性。普通人机模式的原有策略不受影响。
...
# 本地通用 AI：真实 WindBot 回归、素材求和穷举校验、新旧战斗局面对比

...
5. **开始挑战**使用本体的本地决斗服务器与故事模式自己的 WindBot 通用策略，载入玩家为该角色保存的实际卡组。
   默认故事 AI 完整复刻人机模式「P2-自选卡组」的 `LuckyExecutor` 行为，插件内的独立实现位于 `Runtime/Features/StoryMode/StoryLuckyExecutor.cs`。也可通过 `ygo-ai` 接入 OpenAI 兼容大模型；故事页面的「模型设置」可编辑 API 地址、密钥并拉取选择模型 ID。模型异常时由该本地策略接管。配置、架构与测试见 [STORY-AI.md](STORY-AI.md)。
